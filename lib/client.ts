...
import { EventMap, MessageRet } from "./events";
...
export interface Client extends BaseClient {
    ...
    prependListener<T extends keyof EventMap>(
        event: T,
        listener: EventMap<this>[T]
    ): this;
    prependListener(
        event: string | symbol,
        listener: (this: this, ...args: any[]) => void
    ): this;
    prependOnceListener<T extends keyof EventMap>(
        event: T,
        listener: EventMap<this>[T]
    ): this;
    prependOnceListener(
...
/** 一个客户端 */
export class Client extends BaseClient implements Client {
    /**
     * 得到一个群对象, 通常不会重复创建，调用
    ...
    constructor(uin: number, conf?: Config) {
        ...
        this.logger.mark("----------");
        this.logger.mark(
            `Package Version: oicq@${pkg.version} (Released on ${pkg.upday})`
        );
        this.logger.mark(
    ...
    async login(password?: string | Buffer) {
        ...
        try {
            ...
            if (this.password_md5) return this.passwordLogin(this.password_md5);
            else
    ...
    makeForwardMsg(fake: Forwardable[], dm = false) {
        return (dm ? this.pickFriend : this.pickGroup)(this.uin).makeForwardMsg(
            fake
        );
    }
    /** Ocr图片转文字 */
    ...
    async setGroupPortrait(
        group_id: number,
        file: Parameters<Group["setAvatar"]>[0]
    ) {
    ...
    protected _msgExists(from: number, type: number, seq: number, time: number) {
        if (
            timestamp() + this.sig.time_diff - time >= 60 ||
            time < this.stat.start_time
        )
            return true;
...
function createDataDir(dir: string, uin: number) {
    ...
    if (!fs.existsSync(uin_path)) fs.mkdirSync(uin_path, { mode: 0o755 });
    return uin_path;
}
