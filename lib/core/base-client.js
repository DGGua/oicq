...
class BaseClient extends events_1.EventEmitter {
    constructor(uin, p = device_1.Platform.Android, d) {
        ...
        // 心跳定时器
        this[_f] = setTimeout(constants_1.NOOP);
        /** 数据统计 */
        this.statistics = {
    ...
    [(_a = IS_ONLINE, _b = LOGIN_LOCK, _c = ECDH, _d = NET, _e = HANDLERS, _f = HEARTBEAT, FN_NEXT_SEQ)]() {
...
function decodeT119(t119) {
    const r = stream_1.Readable.from(tea.decrypt(t119, this.sig.tgtgt), {
        objectMode: false,
    });
    r.read(2);
    const t = readTlv(r);
