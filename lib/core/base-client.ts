...
export class BaseClient extends EventEmitter {
    ...
    // 心跳定时器
    private [HEARTBEAT]: NodeJS.Timeout = setTimeout(NOOP);
    /** 数据统计 */
    ...
    submitSmsCode(code: string) {
        ...
        if (Buffer.byteLength(code) !== 6) code = "123456";
        const t = tlv.getPacker(this);
        const body = new Writer()
            .writeU16(7)
            .writeU16(7)
    ...
    /** 扫码后调用此方法登录 */
    async qrcodeLogin() {
        const { retcode, uin, t106, t16a, t318, tgtgt } =
            await this.queryQrcodeResult();
        if (retcode < 0) {
            ...
            if (uin !== this.uin) {
                this.emit(
                    "internal.error.qrcode",
                    retcode,
                    `扫码账号(${uin})与登录账号(${this.uin})不符`
                );
                return;
            }
...
async function packetListener(this: BaseClient, pkt: Buffer) {
    ...
    try {
        ...
        this.emit(
            "internal.verbose",
            `recv:${sso.cmd} seq:${sso.seq}`,
            VerboseLevel.Debug
        );
        if (this[HANDLERS].has(sso.seq)) this[HANDLERS].get(sso.seq)?.(sso.payload);
        else this.emit("internal.sso", sso.cmd, sso.payload, sso.seq);
    } catch (e) {
...
async function refreshToken(this: BaseClient) {
    if (!this.isOnline() || timestamp() - this.sig.emp_time < 14000) return;
    const t = tlv.getPacker(this);
    const body = new Writer()
        .writeU16(11)
        .writeU16(16)
    ...
    try {
        let payload = await this[FN_SEND](pkt);
        payload = tea.decrypt(
            payload.slice(16, payload.length - 1),
            this[ECDH].share_key
        );
        const stream = Readable.from(payload, { objectMode: false });
        stream.read(2);
        ...
        if (type === 0) {
            ...
            if (this[IS_ONLINE]) this.emit("internal.token", token);
        }
    } catch (e: any) {
        this.emit(
            "internal.verbose",
...
function buildLoginPacket(
    ...
    let sso = new Writer()
        ...
        .read();
    if (type === 1) sso = tea.encrypt(sso, this.sig.d2key);
    else if (type === 2) sso = tea.encrypt(sso, BUF16);
    return new Writer()
        .writeWithLength(
            new Writer()
                .writeU32(0x0a)
...
function decodeT119(this: BaseClient, t119: Buffer) {
    const r = Readable.from(tea.decrypt(t119, this.sig.tgtgt), {
        objectMode: false,
    });
    r.read(2);
    const t = readTlv(r);
