...
class Converter {
    ...
    share(elem) {
        ...
        title = (0, common_1.escapeXml)(title);
        const data = `<?xml version="1.0" encoding="utf-8"?>
		<msg templateID="12345" action="web" brief="[分享] ${title}" serviceID="1" sourceName="QQ浏览器" url="${(0, common_1.escapeXml)(url)}"><item layout="2">${image ? `<picture cover="${(0, common_1.escapeXml)(image)}"/>` : ""}<title>${title}</title><summary>${content ? (0, common_1.escapeXml)(content) : title}</summary></item><source action="app" name="QQ浏览器" icon="http://url.cn/PWkhNu" i_actionData="tencent100446242://" a_actionData="com.tencent.mtt" appid="100446242" url="http://url.cn/UQoBHn"/></msg>`;
        this.xml({
            type: "xml", data, id: 1
        });
    }
    ...
    reply(elem) {
        const { id } = elem;
        if (id.length > 24)
            this.quote({ ...(0, message_1.parseGroupMessageId)(id), message: "[消息]" });
        else
            this.quote({ ...(0, message_1.parseDmMessageId)(id), message: "[消息]" });
    }
    /** 转换为分片消息 */
    toFragments() {
        this.elems.pop();
